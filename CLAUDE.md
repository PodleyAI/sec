...
Call sites: `cikNameBulkWriter.ts`, `Form8KEventReplace.ts`, `SpacDealReplace.ts` (writes); `feedFilings.ts`, `personObservationTitleBulkReader.ts` (reads). The raw **DDL** in `setupAllDatabases.ts` / `resetAllDatabases.ts` is not a fast path and keeps its own `isDryRun()` guard.
