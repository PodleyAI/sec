...
export interface ExecutiveCompensationEvalOptions {
...
export type ExecutiveCompensationBucket = "hit-agree" | "hit-disagree" | "miss" | "empty" | "skip";

export interface ExecutiveCompensationLineScore {
  readonly person_name: string;
  readonly fiscal_year: number | null;
  readonly salary: number | null;
}

export interface ExecutiveCompensationCase {
  readonly extractor_id: string;
  readonly accession_number: string;
  readonly cik: number | null;
  readonly bucket: ExecutiveCompensationBucket;
  readonly cachePath: string | undefined;
  readonly parsed?: readonly ExecutiveCompensationLineScore[];
  readonly stored?: readonly ExecutiveCompensationLineScore[];
  readonly reason?: string;
}

export interface ExecutiveCompensationReport {
  readonly cases: readonly ExecutiveCompensationCase[];
  readonly counts: Record<ExecutiveCompensationBucket, number>;
}

export async function runExecutiveCompensationEval(
  options: ExecutiveCompensationEvalOptions = {}
): Promise<ExecutiveCompensationReport> {
  if (!globalServiceRegistry.has(SEC_RAW_DATA_FOLDER)) {
    throw new SecCliConfigurationError(
      "SEC_RAW_DATA_FOLDER is not set; executive-compensation eval reads accessiondocs and does not fetch EDGAR"
    );
  }
  ...
  const cases: ExecutiveCompensationCase[] = [];
  ...
  const counts: Record<ExecutiveCompensationBucket, number> = {
    "hit-agree": 0,
    "hit-disagree": 0,
    miss: 0,
    empty: 0,
...
async function loadStored(): Promise<Map<string, ExecutiveCompensationLineScore[]>> {
  const rows = await new ExecutiveCompensationRepo().listAll();
  const people = await new PersonObservationRepo().listAll();
  const byId = new Map(people.map((p) => [p.observation_id, p]));
  const storedByKey = new Map<string, ExecutiveCompensationLineScore[]>();
  for (const r of rows) {
    const key = `${r.extractor_id}\t${r.accession_number}`;
    const person = r.observation_id !== null ? byId.get(r.observation_id) : undefined;
    const person_name = displayName(person);
    const arr = storedByKey.get(key) ?? [];
    arr.push({ person_name, fiscal_year: r.fiscal_year, salary: r.salary });
...
async function scoreCase(
  ...
  stored: readonly ExecutiveCompensationLineScore[]
): Promise<ExecutiveCompensationCase> {
  const base = {
  ...
  const parsedRows = parseSummaryCompensationTable(text);
  const parsed: ExecutiveCompensationLineScore[] = parsedRows.map((r) => ({
...
export function bucketWhenParserEmpty(args: {
  ...
  if (!hasSummaryCompensationTable(args.sectionText)) {
...
function scoredEqual(
  a: readonly ExecutiveCompensationLineScore[],
  b: readonly ExecutiveCompensationLineScore[]
): boolean {
  const keyOf = (r: ExecutiveCompensationLineScore): string =>
    `${nameKey(r.person_name)}\t${r.fiscal_year ?? ""}\t${r.salary ?? ""}`;
