...
describe("CanonicalCompanyRepo", () => {
  ...
  it("findByResolverAndCik scopes by resolver_version", async () => {
    await repo.create(
      makeRow({ canonical_company_id: "uuid-1", resolver_version: "1.0.0", cik: 1234 })
    );
    await repo.create(
      makeRow({ canonical_company_id: "uuid-2", resolver_version: "2.0.0", cik: 1234 })
    );
  ...
  it("findByResolverAndCrd scopes by resolver_version", async () => {
    await repo.create(
      makeRow({ canonical_company_id: "uuid-1", resolver_version: "1.0.0", crd_number: "CRD-001" })
    );
    await repo.create(
      makeRow({ canonical_company_id: "uuid-2", resolver_version: "2.0.0", crd_number: "CRD-001" })
    );

    expect((await repo.findByResolverAndCrd("1.0.0", "CRD-001"))?.canonical_company_id).toBe(
      "uuid-1"
    );
    expect((await repo.findByResolverAndCrd("2.0.0", "CRD-001"))?.canonical_company_id).toBe(
      "uuid-2"
    );
  ...
  it("findByResolverAndName scopes by resolver_version", async () => {
    ...
    await repo.create(
      makeRow({
        canonical_company_id: "uuid-2",
        resolver_version: "2.0.0",
        normalized_name: "acme corp",
      })
    );

    expect((await repo.findByResolverAndName("1.0.0", "acme corp"))?.canonical_company_id).toBe(
      "uuid-1"
    );
    expect((await repo.findByResolverAndName("2.0.0", "acme corp"))?.canonical_company_id).toBe(
      "uuid-2"
    );
