...
import { CompanyIdentityLinkRepo } from "./CompanyIdentityLinkRepo";
import {
...
describe("CompanyIdentityLinkRepo", () => {
  let storage: InMemoryTabularStorage<
    typeof CompanyIdentityLinkSchema,
    typeof CompanyIdentityLinkPrimaryKeyNames,
    CompanyIdentityLink
  >;
  let repo: CompanyIdentityLinkRepo;

  beforeEach(() => {
    storage = makeStorage();
    repo = new CompanyIdentityLinkRepo({ companyIdentityLinkRepository: storage });
  });
  ...
  it("upsert is idempotent on PK (same observation_id+resolver_version overwrites, row count stays 1)", async () => {
    ...
    expect(found?.canonical_company_id).toBe("canon-uuid-b");
  });
  ...
  it("cross-version coexistence: same observation with two resolver_versions → two rows, each getForObservation returns the right one", async () => {
    ...
    expect(v1?.canonical_company_id).toBe("canon-v1");

    const v2 = await repo.getForObservation(42, "2.0.0");
    expect(v2?.canonical_company_id).toBe("canon-v2");
  });
