...
import { runSpacProfileEval } from "../../eval/runSpacProfileEval";
...
export type EvalSpacProfileTaskInput = Static<ReturnType<typeof InputSchema>>;
...
export type EvalSpacProfileTaskOutput = Static<ReturnType<typeof OutputSchema>>;

/**
 * Scores the deterministic SPAC profile parser against stored rows using
 * on-disk accession docs. Never fetches EDGAR.
 */
export class EvalSpacProfileTask extends Task<EvalSpacProfileTaskInput, EvalSpacProfileTaskOutput> {
  static readonly type = "EvalSpacProfileTask";
  static readonly category = "SEC";
  static readonly title = "Evaluate spac profile";
  ...
  async execute(
    input: EvalSpacProfileTaskInput,
    context: IExecuteContext
  ): Promise<EvalSpacProfileTaskOutput> {
    const report = await runSpacProfileEval({
      extractorId:
    ...
    return report as unknown as EvalSpacProfileTaskOutput;
  }
}
