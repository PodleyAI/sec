...
describe("Form_8_K", () => {
  ...
  describe("parsing real SEC EDGAR 8-K filings", () => {
    it("should parse all 8-K files from mock_data directory", async () => {
      ...
      console.log(`Found ${allFiles.length} Form 8-K files to test (${htmFiles.length} HTM, ${xmlFiles.length} XML)`);
      ...
      expect(successfulFiles.length).toBe(allFiles.length);
    });
    ...
    it("should return empty object for all HTML/XHTML primary documents", async () => {
      ...
      for (const file of htmFiles) {
        ...
        // HTML/XHTML docs (including inline XBRL) should return empty object
    ...
    it("should parse XML edgarSubmission documents correctly", async () => {
      ...
      for (const file of xmlFiles) {
        ...
        // XML documents with edgarSubmission should have structured data
  ...
  describe("comprehensive storage with real SEC filings", () => {
    it("should parse and store all 8-K files with their filing metadata", async () => {
      ...
      console.log(`\nStorage Summary: ${totalEventsStored} events stored across ${htmFiles.length} filings`);
    });

    it("should store correct item descriptions for all event items", async () => {
      const mockDataDir = join(__dirname, "mock_data", "form-8k");

      // Process all filings with metadata
      for (const [accessionNumber, metadata] of Object.entries(FILING_METADATA)) {
        ...
        try {
          ...
          // Skip files that don't exist (XML mock files)
          continue;
        }
      }

      // Verify all stored events have valid item descriptions
      for (const [accessionNumber, metadata] of Object.entries(FILING_METADATA)) {
        const events = await eventRepo.getEventsByAccession(metadata.cik, accessionNumber);
  ...
  describe("item type coverage", () => {
    ...
    it("should handle filings with multiple items across categories", async () => {
      ...
      // Tesla - 5.02, 5.07, 9.01 (spans items from sections 5 and 9)
      await processForm8K({
        cik: 1318605,
  ...
  describe("cross-entity querying", () => {
    it("should retrieve events by CIK across multiple filings", async () => {
      // Store events for multiple Apple filings
    ...
    it("should retrieve events by item code across multiple companies", async () => {
      // Store events for Apple, Tesla, and Alphabet - all have 2.02
      for (const accessionNumber of [
      ...
      expect(ciks.has(320193)).toBe(true); // Apple
      expect(ciks.has(1318605)).toBe(true); // Tesla
      expect(ciks.has(1652044)).toBe(true); // Alphabet
    });
  });
  ...
  describe("edge cases", () => {
    ...
    it("should deduplicate items from filing metadata and XML", async () => {
      // Simulate XML with items that overlap with filing metadata
  ...
  <formData>
    <items>
      ...
      await processForm8K({
        ...
        filing_date: "2025-01-15",
        form: "8-K",
        items: "2.02,9.01", // Same items as in XML
        report_date: "2025-01-15",
        form8K,
      });

      const events = await eventRepo.getEventsByAccession(320193, "test-dedup-items");
      expect(events.length).toBe(2); // Deduplicated, not 4
    });
  ...
  <formData>
    <items>
      ...
      await processForm8K({
        ...
        filing_date: "2025-01-15",
        form: "8-K",
        items: "9.01", // Different item than in XML
        report_date: "2025-01-15",
        form8K,
      });

      const events = await eventRepo.getEventsByAccession(320193, "test-merge-items");
      expect(events.length).toBe(2); // One from each source
  ...
  <formData>
    ...
    <periodOfReport>2025-01-10</periodOfReport>
      ...
      await processForm8K({
        ...
        form: "8-K",
        items: "2.02",
        report_date: "2025-01-15", // This should be overridden by XML periodOfReport
        form8K,
      });

      const events = await eventRepo.getEventsByAccession(320193, "test-period-override");
      expect(events[0].report_date).toBe("2025-01-10"); // From XML, not filing metadata
    });
    ...
    it("should handle unknown item codes gracefully", async () => {
      ...
      expect(events[0].item_description).toBeNull(); // Unknown item has no description
    });
  });

  describe("XML edgarSubmission parsing and storage", () => {
    it("should parse and store from XML with signatures", async () => {
      const xml = `<?xml version="1.0" encoding="UTF-8"?>
<edgarSubmission>
  ...
  <formData>
    ...
    <signatureBlock>
      <signature>
        <signatureName>Luca Maestri</signatureName>
      ...
      const events = await eventRepo.getEventsByAccession(320193, "test-xml-sig");
      expect(events.length).toBe(2);

      // Verify signature was stored as a person
      const allPersons = await personRepo.personRepository.getAll();
      const signer = allPersons?.find(
        (person) => person.first === "Luca" && person.last === "Maestri"
      );
      expect(signer).toBeDefined();

      // Verify signature relationship
      const relations = await personRepo.personEntityJunctionRepository.query({
        cik: 320193,
        relation_name: "form-8k:signature",
      });
      expect(relations?.length || 0).toBe(1);
      expect(relations?.[0]?.titles).toContain("Chief Financial Officer");
    });

    it("should handle multiple signatures in XML", async () => {
      const xml = `<?xml version="1.0" encoding="UTF-8"?>
<edgarSubmission>
  <submissionType>8-K</submissionType>
  <formData>
    ...
    <signatureBlock>
      ...
      <signature>
        ...
        <signatureTitle>Secretary</signatureTitle>
      </signature>
    </signatureBlock>
  </formData>
</edgarSubmission>`;
      ...
      await processForm8K({
        cik: 789019,
        accession_number: "test-multi-sig",
        filing_date: "2025-01-15",
        form: "8-K",
        items: "5.02",
        report_date: "2025-01-15",
      ...
      const janeSmith = allPersons?.find(
        (person) => person.first === "Jane" && person.last === "Smith"
      );
      expect(janeSmith).toBeDefined();

      const johnDoe = allPersons?.find(
        (person) => person.first === "John" && person.last === "Doe"
      );
      expect(johnDoe).toBeDefined();
    });

    it("should store company signers from XML correctly", async () => {
      const xml = `<?xml version="1.0" encoding="UTF-8"?>
<edgarSubmission>
  <submissionType>8-K/A</submissionType>
  <formData>
    <items>
      <item>1.01</item>
    </items>
    <signatureBlock>
      <signature>
        <signatureName>Acme Holdings LLC</signatureName>
        <signatureTitle>General Partner</signatureTitle>
      </signature>
    </signatureBlock>
  </formData>
</edgarSubmission>`;

      const form8K = await Form_8_K.parse("8-K/A", xml);

      await processForm8K({
        cik: 789019,
        accession_number: "test-company-sig",
      ...
      // "Acme Holdings LLC" should be stored as a company, not a person
      const allCompanies = await companyRepo.companyRepository.getAll();
      const acme = allCompanies?.find((c) => c.company_name.includes("Acme Holdings"));
  ...
  describe("Form_8_K_ITEMS constant", () => {
    it("should have entries for all major item sections", async () => {
      // Verify coverage of all 8 main sections
      expect(Form_8_K_ITEMS["1.01"]).toBeDefined(); // Section 1
      expect(Form_8_K_ITEMS["2.01"]).toBeDefined(); // Section 2
      expect(Form_8_K_ITEMS["3.01"]).toBeDefined(); // Section 3
      expect(Form_8_K_ITEMS["4.01"]).toBeDefined(); // Section 4
      expect(Form_8_K_ITEMS["5.01"]).toBeDefined(); // Section 5
      expect(Form_8_K_ITEMS["6.01"]).toBeDefined(); // Section 6
      expect(Form_8_K_ITEMS["7.01"]).toBeDefined(); // Section 7
      expect(Form_8_K_ITEMS["8.01"]).toBeDefined(); // Section 8
      expect(Form_8_K_ITEMS["9.01"]).toBeDefined(); // Section 9
    });
